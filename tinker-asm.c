...
*/
// Instruction metadata used by the parser and encoder.
...
void validateLiteral(char* literal, int negativeAllowed)
{
    ...
    if (literal[0] == ':')
    {
        error("label cannot be used as a literal");
    }
    char* end;
    ...
    if (negativeAllowed)
    {
        ...
        {
            error("literal is outside the representable range");
        }
    }
    else
        {
...
void processLD(char* line, FILE* intermediate, memoryLabels* labels, int numLabels)
{
    ...
    if (parameter[0] != ':')
    {
        ...
        // not valid
        if (*end != '\0') {
            error("invalid literal in ld instruction");
        }
    }
    else // label case
    {
        ...
        {  error("label does not have an associated memory address");
    ...
    fprintf(intermediate, "\taddi %s, %" PRIu64 "\n", rd, values[0]);
...
void handleData(char* line, FILE* intermediate)
{
   line = line + 1;
   trim(line);
...
void buildLabelTable(FILE* input, memoryLabels* labels, int* numLabels)
{
    ...
    while (fgets(line, sizeof(line), input)) {
        ...
        else if (line[0] == '\t') //instruction
        {
            ...
            {
               error("instruction appears before a code or data section");
            }
        }
...
void processFile(FILE* filePath, FILE* intermediate,
...
void convertEverythingElse(Requirements* requirement, uint32_t* curr, char *operators)
{
    ...
    if (requirement->numParameters == 3 && requirement->usesLiteral == 0) {
        ...
        *curr |= (literalNum & 0xFFF);
        ...
        {   // mov (rd)(L), rs case
            char r1[16], r2[16], literal[64];
            sscanf(operators, " (%[^)])(%[^)]), %s", r1, literal, r2);
        ...
        *curr |= (literalNum & 0xFFF);
        ...
        else
        {
            // mov rd, (rs)(L) case
            char r1[16], r2[16], literal[64];
            sscanf(operators," %[^,], (%[^)])(%[^)])", r1, r2, literal);
...
void passTwo(char* intermediatePath, char* outputPath)
{
    ...
    {   count++;
        ...
        if (strncmp(line, ".data", 5) == 0)
           ...
           {
              error("invalid line in code section");
           }
    }
    ...
    {   count++;
        ...
        if (strncmp(line, ".data", 5) == 0)
           ...
           {
              error("invalid line in data section");
           }
    }
...
int main(int argc, char * argv[])
{
    if (argc != 3)
    {
        error("usage: tinker-asm <input.tk> <output.tko>");
    }
    ...
    rewind(filePath);
    processFile(filePath, intermediate, labels, numLabels);
    intermediate = NULL;
