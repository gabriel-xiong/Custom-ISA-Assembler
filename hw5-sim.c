...
void processPriv(uint32_t instruction) {
    ...
    if (literal == 0x0)
    ...
    else if (literal == 0x3)
        {
            ...
            uint64_t input; errno = 0;
            
            if (sscanf(check, "%lu", &input) != 1) 
                error();
            if (errno == ERANGE)
                 error();
...
void processMovWrite(uint32_t instruction) {
    ...
    uint64_t newAdd = (int64_t) signedAdd;
    if (newAdd + 8 > totalMemory || newAdd < 0)
        error();
...
{   
    ...
    // initialize to zero values --> might be unecessary but good to be safe? 
    ...
    // read header 
    fread(head, sizeof(uint64_t), 5, filePath); 
    ...
    fread(memory + code_start , 1, code_size, filePath); 
    fread(memory + data_start, 1 , data_size, filePath); 
